...
class NERModel(LanguageModel):
  ...
  def add_placeholders(self):
    ...
    self.input_placeholder = tf.placeholder(tf.int32, 
                                            (None, self.config.window_size),
                                            'input')
    self.labels_placeholder = tf.placeholder(tf.float32, 
                                             (None, self.config.label_size),
                                             'labels')
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ### YOUR CODE HERE
      L = tf.get_variable('L', (len(self.wv), self.config.embed_size))
  ...
  def add_loss_op(self, y):
    ...
    """
    ### YOUR CODE HERE
    CE = tf.nn.softmax_cross_entropy_with_logits(y, self.labels_placeholder)
